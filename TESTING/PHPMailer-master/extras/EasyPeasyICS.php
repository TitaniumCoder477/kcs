<?php
/**
 * EasyPeasyKCS Simple KCS/vCal data generator.
...
/**
 * Class EasyPeasyKCS.
 * Simple KCS data generator
 * @package phpmailer
 * @subpackage easypeasykcs
 */
class EasyPeasyKCS
{
    /**
    ...
    {
        if (empty($uid)) {
            $uid = md5(uniqid(mt_rand(), true)) . '@EasyPeasyKCS';
        }
        $event = array(
...
END:VCALENDAR';
        ...
        if ($output) {
            ...
            header('Content-Disposition: inline; filename=' . $filename . '.kcs');
            echo $kcs;
        }
        return $kcs;
    }
