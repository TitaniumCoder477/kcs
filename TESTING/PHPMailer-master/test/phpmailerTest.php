...
EOT;
    ...
    {
        //Standalone KCS tests
        require_once '../extras/EasyPeasyKCS.php';
        $KCS = new EasyPeasyKCS("PHPMailer test calendar");
        $this->assertNotEmpty(
            $KCS->addEvent(
        ...
        $events = $KCS->getEvents();
        $this->assertEquals(2, count($events), 'Event count mismatch');
        $KCS->clearEvents();
        $events = $KCS->getEvents();
        $this->assertEquals(0, count($events), 'Event clearing failed');
        $KCS->setName('test');
        $this->assertEquals('test', $KCS->getName(), 'Setting KCS name failed');
        $this->assertNotEmpty($KCS->render(false), 'Empty calendar');
        ...
        $this->Mail->isHTML(true);
        $this->buildBody();
        $this->Mail->Ical = $KCS->render(false);
        $this->assertTrue($this->Mail->send(), $this->Mail->ErrorInfo);
    }
